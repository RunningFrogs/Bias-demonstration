...
def prognose_automated():
    if not os.path.exists(paths.path_test_data):
        print(f'{paths.path_test_data} does not exist.')
        return
    ...
    input_data = pd.read_csv(paths.path_test_data)
    ...
    input_data.to_csv(paths.path_prognosed_data, index=False)

    print(f'Predictions saved at{paths.path_prognosed_data}.')
