...
def generate_graphic_frequency_distribution(input_path, result_path):
    logging.info('Starting graphic frequency distribution generation process.')
    if not os.path.exists(input_path):
        logging.error(f'{input_path} does not exist.')
        print(f'{input_path} does not exist.')
        return
    else:
        logging.info(f'{input_path} found, proceeding with processing.')
    ...
    logging.info('Gender values translated to German.')

    # Check if "Salary" exists, otherwise remove it from translation
    if 'Salary' not in df.columns:
        column_translations.pop('Salary', None)
        logging.info('"Salary" column not found, removed from translations.')
    ...
    for column in df.columns:
        ...
        if df[column].dtype == 'object':
            ...
            logging.info(f'Bar chart created for {column}.')
        else:
            ...
            logging.info(f'Histogram created for {column}.')
        ...
        plt.close()
    logging.info('Graphic frequency distribution generation process completed.')


def generate_text_frequency_distribution(input_path, result_path):
    logging.info('Starting text frequency distribution generation process.')
    df = pd.read_csv(input_path)
    logging.info('CSV file loaded successfully.')

    # Remove empty rows
    df = df.dropna(how='all')
    logging.info('Empty rows removed.')
    ...
    if 'Salary' not in df.columns:
        ...
        logging.info('"Salary" column not found, removed from the dataframe.')

    # Create a directory for the results if it does not exist
    output_dir = f'{result_path}/frequency_distribution/text/'
    os.makedirs(output_dir, exist_ok=True)
    logging.info(f'Output directory {output_dir} created/verified.')
    ...
    for column in df.columns:
        ...
        with open(txt_file_path, 'w') as file:
            logging.info(f'Writing distribution data to {txt_file_path}.')
            ...
            file.write('\n')
    logging.info('Text frequency distribution generation process completed.')
